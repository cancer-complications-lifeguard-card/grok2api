...
Adapted for tqzhr Cloudflare deployment architecture.
"""
...
class ResponsesService:
    ...
    async def create(
        *,
        ...
        # 映射 reasoning_effort 到 thinking
        thinking = None
        if reasoning_effort:
        ...
        if not stream:
            ...
            if normalized_tools and content:
                text_content, tool_calls_parsed = parse_tool_calls(content, normalized_tools)
        ...
        async def _stream() -> AsyncGenerator[str, None]:
            ...
            async for chunk in result:
                if not chunk:
                    continue
                line = chunk.strip() if isinstance(chunk, str) else chunk
                if not line:
                    continue
                # 去除 SSE 前缀
                if isinstance(line, str):
                    ...
                    if line == "[DONE]":
                        continue
                try:
                    data = orjson.loads(line) if isinstance(line, str) else orjson.loads(line)
